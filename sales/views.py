...
class SalesRecordAggregateView(ListAPIView):
    ...
    def get_queryset(self):
        ...
        """"
        ORM does not reuse total_sales=Sum("total_sales_amount")
        so in SQL we have SUM total_sales_amount twice which is suboptimal.
        Also there is one redundant cast to Numeric before casting to Real.
        There might be a way to do this optimization with ORM but if not writing raw sql will solve the issue.
        """
        return queryset
